...
| `src/ledger/` | Append-only live savings ledger: fail-open writer (`write.ts`), tolerant reader (`read.ts`), aggregation and the HTML/terminal report rows (`report.ts`), shared project labelling (`project.ts`). |
...
| `src/index.ts` | Public API surface (engine, tokens, transcripts, and the ledger/project-labelling re-exports the VS Code extension consumes). |

Published to npm as `@astudioplus/compressor` (`npm install -g @astudioplus/compressor`); a source checkout builds with `npm install && npm run build`.
...
### Recovery-read budget (`src/hook/recovery.ts`)

Targeted reads (`offset`/`limit`) pass through uncompressed by design — recovery of truncated content is a feature. The budget keeps recovery from becoming unbounded re-acquisition: when the hook truncates a Read of a file, it notes that (session-scoped state under `os.tmpdir()/compressor-recovery/`, override `COMPRESSOR_RECOVERY_DIR`); the first **3** subsequent targeted reads of that file pass through untouched, and further ones are compressed like untargeted reads (their markers still say what was cut). Tune with `COMPRESSOR_RECOVERY_BUDGET=<n>` (`0` = compress all targeted reads of truncated files), disable with `COMPRESSOR_NO_RECOVERY_BUDGET=1`, or per-invocation with the `--recovery-budget <n|off>` flag on the hook entries (argv overrides env — this is how benchmark arms vary it). Everything is fail-open: missing/corrupt state, missing session id, or any fs error means passthrough; the counter survives re-truncation (resetting it would re-open the loop); state writes share the 250 ms settle cap. Measured note: in the first valid hook A/B (run bench-20260612-110402) only 8 recovery reads occurred across 15 hook-on cells — the budget never escalated; its effect on behavior is so far unmeasured.

### Hook command styles (`src/paths.ts`)

Installed hook commands come in two forms, chosen by `detectHookCommandStyle` (or forced via `init --hook-command`): **absolute** (`node "<root>/dist/hook.js" --mode X`) for source checkouts — dogfooding tracks the dev build — and **relocatable** (`compressor-hook --mode X`, the package.json bin running the bundle directly, no CLI startup) when npm-installed with the bins on PATH. Relocatable commands make a committed `.github/hooks/compressor.json` work on any machine with the package installed. Ownership predicates claim both forms, so re-running `init` upgrades old absolute installs in place; benchmark cells always pin absolute so measurements exercise the local build.
...
| opencode | owned plugin `.opencode/plugins/compressor.js` (mode marker + `COMPRESSOR_MODE` prologue + self-contained bundle of the engine — in-process `tool.execute.after` mutates `output.output` directly; ledger agent `opencode`; recovery budget via in-band `sessionID`) | `~/.config/opencode/plugins/compressor.js` | owned file | file deleted; empty `plugins`/`.opencode`/`.config/opencode` dirs pruned (bounded). Instructions come via agents-md (OpenCode reads it natively). Plugin format doc-verified 2026-06-12; not yet live-verified — status says so |
...
**Event schema** (`src/ledger/write.ts`): `{ts, agent: 'claude-code' | 'copilot' | 'opencode' | 'vscode', tool, mode, charsIn, charsOut, estTokensIn, estTokensOut, transforms: [ids], project?}`, appended as JSONL to monthly files under `~/.compressor/ledger/` (override: `COMPRESSOR_LEDGER_DIR`). `project` is optional in both directions (older ledgers read unchanged, older readers ignore it), so the field needs no migration.

- **Privacy posture** — events carry sizes and transform ids only. No file paths, no content, nothing leaves the machine. The one identifying field is the optional `project` label, and it is never a path: writers record a keyed digest by default, and a writer that records a readable name instead must make that an explicit opt-in, because every writer shares one ledger and the weakest one sets the privacy of the whole file.
- **Project attribution** (`src/ledger/project.ts`) - one labeller for every writer into the ledger (the CLI hooks and the VS Code extension), so one folder gets one label whichever tool recorded the event. The label is a keyed SHA-256 digest of the canonicalized workspace path, prefixed `#`; `COMPRESSOR_PROJECT_LABEL=name` opts into the clear-text folder name instead (matching the extension's `compressor.projectLabel`), capped at `PROJECT_LABEL_MAX` (64) and never the absolute path. The key lives at `~/.compressor/project-salt` (owner-only, override `COMPRESSOR_PROJECT_SALT`, which names the key *file* and so must be an absolute path - a relative path or a key pasted in place of a path is ignored and the default location is used, rather than creating a key file relative to whatever directory the hook happened to run in) and deliberately **not** inside the ledger directory: the ledger is the shareable unit, so a shared report carries digests the recipient cannot test against guessed project names. Creation is exclusive and a loser adopts the winner's key, so a hook and the editor racing at first use cannot split one project into two rows. Reads are synchronous because a hook process often lives for a single tool call. Fail-open throughout: a key that can be neither read nor created yields no label rather than a brute-forceable unsalted digest, and an unusable label on read drops only the label, not the event's savings data.
- **Reporting the dimension** - both surfaces (the terminal chart and the HTML report) go through one policy, `chartRows` in `src/ledger/report.ts`, so the two cannot drift. Events with no label group under `unattributed`, and rows past `PROJECT_ROW_LIMIT` (12) fold into a single `other` row, since the bar chart is fixed-height-per-row and a machine with many repos would otherwise produce an unbounded SVG. The HTML report grows its "by project" chart only once some event carries a label, so an existing ledger does not gain a chart with one meaningless bar.
...
- **Kill switch** — `COMPRESSOR_NO_LEDGER=1` disables recording before any IO is attempted, labelling included: resolving a project label reads and may create the key, so the switch is one exported predicate (`ledgerDisabled`) consulted at the top of the recording path rather than only at the append, and an opt-out never plants a key in the user's home. The benchmark harness sets it in every cell so synthetic benchmark compressions can never pollute the user's live ledger (and measured cells pay no ledger syscalls).
- **Estimated vs measured** — ledger token numbers come from the cheap estimator (chars/3.5) and every rendering says so: chars are exact, tokens are "estimated — cheap estimator, not billable counts". Measured savings come from `compressor benchmark`; the ledger never claims otherwise. `compressor savings` (`--since`, `--by day|tool|mode|agent|project`, `--html` for a self-contained no-JS report) always states its lookback window, and its empty states distinguish "no ledger at all — hook not installed?" from "no events inside this window — the hook is fine".
...
- **No-cost circuit breaker** (api mode) — cells that report no cost (timeouts, unexpected auth fallback) still bill real money the dollar ceiling cannot see; after 3 consecutive no-cost cells the ceiling is unenforceable and the runner stops scheduling rather than burn the whole grid.
...
- **Same-run arm fan-outs** — experiment arms must share one run, one ceiling, and the same task×trial groups (separate runs give each arm its own truncation point and unbalance the comparison). Three fan-outs exist: `--marker-styles plain,deterrent,informative` (marker-phrasing arms), `--hook-arg-arms '<label>=<args>,…'` (arbitrary per-arm hook arguments — e.g. the recovery-budget A/B: `budget-on=,budget-off=--recovery-budget off`), and `--hook-arms` (hook-on vs hook-off with the instruction pack held constant — the pure compression A/B).
- **Auth modes** — `--auth api` (default) bills `ANTHROPIC_API_KEY` under the dollar ceiling; `--auth subscription` bills the operator's Claude plan via `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`). Cells strip the other mode's credential so billing is deterministic. Subscription runs: the ceiling is `--max-cells` (group-atomic), the no-cost breaker is replaced by a 4-consecutive-errors breaker (dead token / exhausted usage window), progress reports tokens consumed, run meta records `authMode`, and reported cost columns are API-equivalent figures, not dollars billed.
