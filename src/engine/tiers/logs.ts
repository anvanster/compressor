import { tierResult } from './structural.ts';
...
export function filterTestLog(content: string): TierResult {
  ...
  out.splice(markerIdx, 0, `[compressor: ${omitted} passing-test lines omitted]`);
...
export function filterBuildLog(content: string): TierResult {
  const lines = content.split('\n');
  ...
  if (omitted === 0) return { content };
  out.splice(markerIdx, 0, `[compressor: ${omitted} build-log lines omitted]`);
  return tierResult(content, out.join('\n'), 'log-filter');
}
